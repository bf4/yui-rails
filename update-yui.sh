...
then
    ...
    cd ../../..
    perl -pi -w -e "s/YUI_VERSION.*/YUI_VERSION = '$1'/g;" `pwd`/lib/yui_rails/rails/version.rb 
else
    echo "run this with the version number, e.g. ./update_yui.sh 3.8.1"
    echo "this will also update lib/yui_rails/rails/version.rb YUI_VERSION to 3.8.1"
fi
